...
TEST_CASE("Perspective camera controller preserves its initial camera pose", "[camera][controller]") {
    ...
    vshade::renderer::PerspectiveCameraController controller(camera);
...
TEST_CASE("Perspective camera controller validates movement settings", "[camera][controller]") {
    vshade::renderer::Camera camera;
    vshade::renderer::PerspectiveCameraController controller(camera);
    ...
    CHECK_THROWS_AS(
        vshade::renderer::PerspectiveCameraController(
            camera,
